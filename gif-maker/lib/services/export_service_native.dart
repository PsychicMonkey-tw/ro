...
Future<ExportResult> exportNative({
  ...
  if (!purchaseService.canExportFormat(_formatLike(format))) {
    throw ExportException('Format locked in demo mode');
  }

  final effectiveQuality =
      purchaseService.maxQuality(_qualityLike(quality));
  const uuid = Uuid();
  final dir = await getTemporaryDirectory();
  final ext = _extension(format);
  final outputPath = p.join(dir.path, '${uuid.v4()}.$ext');
...
String _photosCommand(
  ...
  final slideSec = (project.durationMs / project.mediaPaths.length / 1000)
      .clamp(0.3, 2.0)
      .toStringAsFixed(2);
  final inputs = project.mediaPaths
      .map((path) => '-loop 1 -t $slideSec -i ${_escapePath(path)}')
      .join(' ');
...
ExportFormatLike _formatLike(ExportFormat format) {
  switch (format) {
    case ExportFormat.gif:
...
ExportQualityLike _qualityLike(ExportQuality quality) {
  switch (quality) {
